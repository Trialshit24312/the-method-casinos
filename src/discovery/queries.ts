...
import { getDiscoveryMeta, setDiscoveryMeta } from '../database/index.js';

const QUERY_CURSOR_KEY = 'search_query_cursor';

function readQueryCursor(): number {
  try {
    return parseInt(getDiscoveryMeta(QUERY_CURSOR_KEY) ?? '0', 10) || 0;
  } catch {
    return 0;
  }
}

function writeQueryCursor(value: number): void {
  try {
    setDiscoveryMeta(QUERY_CURSOR_KEY, String(value));
  } catch {
    /* db not initialized (unit tests) */
  }
}
...
export function buildSearchQueries(deep: boolean): string[] {
  ...
  const cursor = readQueryCursor();
  const start = shuffled.length > 0 ? cursor % shuffled.length : 0;
  const rotated = [...shuffled.slice(start), ...shuffled.slice(0, start)];
  const batch = rotated.slice(0, limit);
  if (shuffled.length > 0) {
    writeQueryCursor((start + limit) % shuffled.length);
  }
  return batch;
}
