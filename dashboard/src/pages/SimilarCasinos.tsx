...
import { Sparkles, Search, Star, ExternalLink, ChevronRight, Dices } from 'lucide-react';
import { api } from '../api';
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <PageHeader
        ...
        subtitle="Pick any casino — we match by features, VPN access, signup style, rating, and redeem options"
      />
      ...
      {selected && !loading && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6 p-4 rounded-xl bg-[#b87333]/10 border border-[#b87333]/25">
            <p className="text-sm text-gray-400">
              Showing casinos similar to{' '}
              <span className="text-white font-semibold">{selected.name}</span>
              {' '}— matched on {selected.features.length} feature tags
            </p>
          </motion.div>
          ...
          {matches.length === 0 ? (
            <div className="glass p-12 text-center">
              ...
              <p className="text-gray-500">No close matches yet. Try another casino or add more entries to the database.</p>
            </div>
          ) : (
      ...
      {!selected && !loading && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass p-8 text-center">
          ...
          <p className="text-gray-500">Select a casino above to discover alternatives with the same vibe — no phone, same games, similar redeem options.</p>
        </motion.div>
      )}
    </div>
  );
