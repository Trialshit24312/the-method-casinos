  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <PageHeader
        ...
        subtitle="Pick a casino — match from catalog by features, or search the free web for more operators like it"
      />
        ...
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6 p-4 rounded-xl bg-[#b87333]/10 border border-[#b87333]/25 flex flex-wrap items-center justify-between gap-3">
            ...
            >
              ...
              {webLoading ? 'Searching web…' : 'Search web for more'}
            </button>
          </motion.div>
          ...
          {webResult && (
            <div className="mb-6 space-y-3">
              <div className="p-4 rounded-lg border border-[#00aeef]/25 bg-[#00aeef]/5 text-sm text-gray-300">
                <p>
                  Web search ({webResult.queries.length} queries): {webResult.webUrlsFound} URLs found · {webResult.analyzed} analyzed ·{' '}
                  <span className="text-emerald-400">{webResult.added} queued for review</span>
                  {webResult.rejected > 0 && ` · ${webResult.rejected} rejected`}
                </p>
                {webResult.added > 0 && (
