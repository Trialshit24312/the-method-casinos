...
export default function ToolsHub() {
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <PageHeader
        icon={<Wrench className="w-6 h-6 text-[#00aeef]" />}
        title="Tools Hub"
        ...
        {quickLinks.map((item, i) => (
          <motion.div key={item.to} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}>
            <Link to={item.to} className="block p-4 rounded-xl border border-[#2a2a35] bg-[#121218]/80
              hover:border-[#00aeef]/40 hover:shadow-[0_0_24px_rgba(0,174,239,0.12)] transition-all h-full text-center group">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-glow p-6 flex items-start gap-4 mb-10 border-[#00aeef]/20">
        <Sparkles className="w-8 h-8 text-[#00aeef] shrink-0" />
        <div>
          <h3 className="font-display font-semibold mb-2 text-white">The Method Workflow</h3>
          <ol className="text-sm text-gray-400 space-y-2 list-decimal list-inside">
            <li>Find a casino on <Link to="/casinos" className="text-[#00aeef] hover:underline">Casinos</Link> — filter by no phone, VPN, slots, etc.</li>
            <li>Use <Link to="/similar" className="text-[#00aeef] hover:underline">Similar Casinos</Link> to find alike sites when you like one platform</li>
            <li>Open a <Link to="/tools/email" className="text-[#00aeef] hover:underline">temp-mail site</Link> for a real working inbox</li>
            ...
            <li>Run <Link to="/discovery" className="text-[#00aeef] hover:underline">Discovery</Link> to expand the database with new sites</li>
          </ol>
        </div>
      </motion.div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-10">
        ...
        <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="glass p-5 flex gap-4">
          <Globe className="w-8 h-8 text-[#00aeef] shrink-0" />
          <div>
