...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen flex flex-col app-background relative overflow-hidden">
      <div className="absolute inset-0 app-background-grid pointer-events-none opacity-60" />

      <header className="relative border-b border-surface-border/80 bg-surface-raised/30 backdrop-blur-md sticky top-0 z-20">
      ...
      <main className="relative flex-1 max-w-6xl mx-auto px-4 py-12 md:py-20 w-full">
        <motion.section {...fadeUp} className="hero-glow text-center mb-16 relative z-10">
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ delay: 0.1 }}
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-glow/25 bg-glow/5 text-glow text-xs mb-6"
          >
            <Zap className="w-3.5 h-3.5" />
            100% free · No API keys · Web search built in
          </motion.div>

          <h1 className="font-display text-4xl md:text-6xl font-bold mb-5 leading-tight">
            <span className="text-gradient-brand">US Sweepstakes Casinos</span>
            <span className="block text-xl md:text-2xl mt-3 font-normal text-gray-400">
              Verified catalog · Similar search · Scam checker
            </span>
          </h1>

          <p className="text-gray-400 max-w-2xl mx-auto mb-10 text-base md:text-lg leading-relaxed">
            Real operators only — no phone signup lists, URL safety checks, signup tools, and free web discovery.
            Same power on Discord with <code className="text-glow/80 text-sm">/similar</code>,{' '}
            <code className="text-glow/80 text-sm">/check</code>, and{' '}
            <code className="text-glow/80 text-sm">/search</code>.
          </p>
