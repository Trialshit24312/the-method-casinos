...
export default function MyList() {
  ...
  useEffect(() => {
    const merged = sessionStorage.getItem('method-guest-merge-notice');
    if (merged && user) {
      showNotice(`Synced ${merged} local favorite${merged === '1' ? '' : 's'} to your account`);
      sessionStorage.removeItem('method-guest-merge-notice');
    }
  }, [user, showNotice]);
