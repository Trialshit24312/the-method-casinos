...
export function isCatalogStale(lastCheckedAt: string | null | undefined, now = Date.now()): boolean {
  if (!lastCheckedAt) return true;
  const ageMs = now - new Date(lastCheckedAt).getTime();
  return ageMs > STALE_CATALOG_DAYS * 24 * 60 * 60 * 1000;
}

export function formatLastChecked(lastCheckedAt: string | null | undefined): string {
